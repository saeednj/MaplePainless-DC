...
protected:
   ...
   typedef struct ListRoot
   {
     ...
     ~ListRoot(){
       ...
       ListElement * actual=head==NULL?NULL:head.load();
       ListElement * next=NULL;
       while(actual!=NULL){
        next=actual->next==NULL?NULL:actual->next.load();
       /*if(actual->clause==NULL){
         printf("%p but it's nil\n",actual->clause);
        }else{
          int n=actual->clause->nbRefs.load();
          if(n<=1)
            printf("%p nRefs=%d\n",actual->clause,actual->clause->nbRefs.load());
            }/
        if(actual->clause!=NULL)ClauseManager::releaseClause(actual->clause);
        delete actual;
        actual=next;
        }*/
     }
